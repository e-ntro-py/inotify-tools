...
# master's tip, so `ensure` runs first thing in a run, while its commit is
# the tip. The same check applies to a release's target_commitish, so the
# release job must not send one. Only a 404 counts as "absent".
...
get() {
	...
	*"(HTTP 404)"*) return 1 ;;
	...
	gh api -X POST "repos/$GITHUB_REPOSITORY/git/refs" -f "ref=refs/tags/$1" -f "sha=$2" --jq .ref ||
		die "creating $1 at $2 failed; a 403 means .github/workflows/ changed on master since this commit"
}
